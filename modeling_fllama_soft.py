...
class FLlamaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # It usually represents the queries or keys in multi-head attention.
        # position_ids: A tensor of shape [batch_size, seq_len], indicating the position index of each token.
        inv_freq_expanded = self.inv_freq[None, :, None].float().expand(position_ids.shape[0], -1, 1)
        position_ids_expanded = position_ids[:, None, :].float()
        # Force float32 since bfloat16 loses precision on long contexts
        # See https://github.com/huggingface/transformers/pull/29285

        # self.inv_freq is a vector that holds the “inverse frequency” terms for each dimension;
        # it’s precomputed based on the base and dim
        # inv_freq_expanded shapes it to [batch_size, dim/2, 1]
        # position_ids_expanded reshapes [batch_size, seq_len] to [batch_size, 1, seq_len]

        # These expansions prepare for a matrix multiply: each token’s position is multiplied by each inverse frequency.

        # Turn off the forced float32 autocast from the rotary embedding
        ...
        # torch.autocast(..., enabled=False) forces float32 precision for the calculation, problem
        # avoiding precision loss when x might be bfloat16 or float16.

        # (inv_freq_expanded @ position_ids_expanded) multiplies each position by each frequency,
        # yielding an angle value (often shape [batch_size, seq_len, dim/2] after transposition).

        # torch.cat((freqs, freqs), dim=-1) duplicates the angle dimension so it matches dim
        # emb.cos() and emb.sin() compute the actual cosine/sine values for each angle

        # These (cos, sin) arrays each match [batch_size, seq_len, dim]

    # This code implements Rotary Position Embeddings in a Llama-like style by precomputing cos & sin for all possible positions up to max_position_embeddings, with an optional scaling_factor.
    # The forward(...) call produces the cos/sin arrays for the batch of positions (on the correct device & dtype) so you can rotate Q/K in self-attention.
...
class FLlamaLinearScalingRotaryEmbedding(FLlamaRotaryEmbedding):
    ...
    # Before delegating to the parent’s forward(...) method, it scales the position_ids by 1 / self.scaling_factor
    # That effectively modifies the “frequency” of the rotation in a linear manner. If scaling_factor > 1, positions get shrunk;
    # if < 1, positions get stretched.
    # After scaling the position IDs, it calls super().forward(...), which is the
    # standard RoPE calculation (computing cos and sin for the adjusted positions).

    # models benefit from changing the rate at which positional angles increase with token index.
    # For example, to handle longer contexts or tune how quickly the rotation changes with each step.
    # The original Llama uses fixed base=10000 and no scaling factor.
    # This subclass adds a straightforward multiplier to the position IDs before applying the RoPE formula.
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # 2) Print shapes right here to confirm final shapes
...
# In Rotary Position Embeddings, not adding a positional embedding to q and k, we
# rotate them in their feature space by angles determined by cos and sin

# This function does exactly that rotation, producing position-aware Q/K vectors used in self-attention.

# in short: apply_rotary_pos_emb takes the original Q/K tensors plus the RoPE cos/sin arrays, rotates Q and K accordingly, and
# returns the position-encoded Q and K for use in attention.
...
class FLlamaAttention(nn.Module):
    ...
    def _apply_headwise_linear(self, x, weight, num_heads):
        ...
        # The original weight was shape (\text{num_heads} * \text{head_dim}, \, \text{hidden_size}).
        # We reshape it into (\text{num_heads}, \, \text{head_dim}, \, \text{hidden_size}).
        # This way, each head can have a slice [head_idx, :, :] that transforms a (head_dim -> hidden_size) space.
        projected = torch.einsum(
            'nbld,nhd->nblh',
            x,
            weight_
        )
        # Essentially, for each head n, we multiply the sub-tensor x[n] (shape (b, l, d)) by the submatrix
        # weight[n] (shape (d, hidden_size))
        # the result has shape (n, b, l, h)
        projected = projected.permute(1, 0, 2, 3)     # (batch_size, n_heads, seq_len, head_dim)
        return projected
        #  the per-head projected result of x


    # x's shape: (\text{num_q_or_kv_heads}, \, \text{batch_size}, \, \text{seq_len}, \, \text{head_dim})
    # This tensor holds the hidden states for multiple heads.
    # (For example, in Q-projection it might be (\text{num_heads}, b, s, \text{head_dim}).)

    # weight
    # Shape: (\text{num_q_or_kv_heads} \times \text{head_dim}, \, \text{hidden_size}).
    # Essentially, each attention head has a block of rows for its linear transformation, flattened into one big matrix.

    # num_heads
    # Typically the same as \text{num_q_or_kv_heads}. It indicates how many slices (heads) there are in x

    # self.head_dim: the dimension of each head.
    # self.hidden_size: the total hidden size.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            # These lines perform headwise linear projections on three different input tensors (q_hidden_states, k_hidden_states, and v_hidden_states) to
            # produce query_states, key_states, and value_states for attention

        past_key_value = getattr(self, "past_key_value", past_key_value)
        cos, sin = self.rotary_emb(value_states, position_ids)
        # Internally, it calculates angles for each token’s position and dimension via a matrix multiply with inv_freq, then
        # does .cos() and .sin() on those angles
        ...
        # The apply_rotary_pos_emb function implements the rotation
        # q_rot = q * cos + rotate_half(q) * sin
        # k_rot = k * cos + rotate_half(k) * sin
        # This is the core of RoPE: each query/key vector is split in half and rotated by angles determined by the position.
        ...
        # Once Q and K are rotated, you do the usual attention steps:
        # attn_weights = (q_rot @ k_rot.transpose(-2, -1)) / sqrt(head_dim)
...
class FLlamaDecoderLayer(nn.Module):
    def __init__(
        ...
        # writer_offset accounts for whether there is an extra “embedding node” at writer index 0.
        # layer_idx is the index of the current layer in the stack
        # num_heads + 1 means: for each layer, we assign one writer slot to each attention head (num_heads) and
        # one slot for that layer’s MLP.
        # - 1 at the end offsets to point exactly to the MLP’s slot (as opposed to the next layer’s first attention head).
        ...
        # Continuous masking
        self.q_read_weights = nn.Parameter(torch.ones_like(self.q_read_log_alphas))
        self.k_read_weights = nn.Parameter(torch.ones_like(self.k_read_log_alphas))
        self.v_read_weights = nn.Parameter(torch.ones_like(self.v_read_log_alphas))
        self.mlp_read_weights = nn.Parameter(torch.ones_like(self.mlp_read_log_alphas))
    ...
    def attn_read(self, x, corr_x=None, embeds=None):
        ...
        # # (w, h)
        # # total number of possible writer nodes that could feed into this layer
        # # total attention heads in the current layer
        #
        ...
        #
        # x_q = torch.einsum("wbsd,wh->hbsd", x, q_z) # (h, b, s, d)
        # # q_read_log_alphas is (w, h)
        # # x_q (num_heads, batch_size, sequence_length, hidden_size)
        # # Dimension 0 (num_heads): Each index here selects one attention head
        # # Dimension 1 (batch_size): For each head, you have a row for each sequence in the batch
        # # if you take x_q[h] for a particular head h, you get a 3D slice of shape (batch_size, sequence_length, hidden_size)
        # # x_q is the result that already combines the writers that were chosen for each head attention in this layer for query.
        # x_k = torch.einsum("wbsd,wh->hbsd", x, k_z) # x (w, b, s, d), k_z (w, h), x_k (h, b, s, d)
        ...
        q_w = self.q_read_weights  # shape (writers, heads) real values
        k_w = self.k_read_weights
        v_w = self.v_read_weights

        # 2) Multiply the writer states by those weights
        # print("q_w", q_w)
        # print("k_w", k_w)
        # print("v_w", v_w)
        x_q = torch.einsum("wbsd,wh->hbsd", x, q_w)
        # print("x_q after weighting:", x_q[0, :, :5, :5])
        x_k = torch.einsum("wbsd,wh->hbsd", x, k_w)
        x_v = torch.einsum("wbsd,wh->hbsd", x, v_w)
        ...
        z_edges_sum = (q_w.sum() + k_w.sum() + v_w.sum()).detach()
        return x_q, x_k, x_v, z_edges_sum
    ...
    # attn_read uses a mask to decide which edges (writer → Q/K/V) are active.
    # For example, q_read_log_alphas (size (\text{num_writers}, \text{num_heads})) produce a mask that selects which previous writer nodes contribute to the queries for each head.

    # attn_write uses a mask to decide whether each attention head (node) as a whole is active.
    # For example, attn_write_log_alphas (size (\text{num_heads},)) produce a mask that turns each entire head “on” or “off.”

    # They both call get_mask(...)
    # attn_read → (Edge-level pruning*) decides which inputs each head sees.
    # attn_write → (Node-level pruning*) decides if a head’s output is actually kept or zeroed out entirely.

    # The entry q_read_log_alphas[w, h] decides whether that writer node w contributes to head h’s queries.
    ...
    def mlp_read(self, x, corr_x=None, embeds=None):
        ...
        # x_z = torch.einsum("wbsd,w->bsd", x, z) # x has shape (w, b, s, d), # z has shape (w,)
        # # for each token (b, s) and hidden dimension d, take the sum of x[w, b, s, d] across all writers w, weighted by z[w]
        # # If z[w] is zero for some writer w,
        # # that writer’s contribution is removed. If z[w] is one, that writer’s hidden state is included in the sum.
        # # z has shape (w, ) one entry per writer (0 or 1, in this case)
        # # This means that for every token (𝑏 × 𝑠) you now have a single (d)-dimensional vector that combines the contributions from all the
        # # writers that were “on” according to z.
        # # The output x_z has shape (b, s, d)
        #
        # if embeds is not None:
        #     x_z = x_z + embeds
        # if corr_x is not None:
        ...
        w = self.mlp_read_weights
        # multiply writer states by w
        x_z = torch.einsum("wbsd, w -> bsd", x, w)

        if embeds is not None:
            x_z += embeds

        z_edges_sum = w.sum().detach()
        return x_z, z_edges_sum
    ...
    def mlp_write(self, residual, x, corr_x=None):
        ...
        # inputs: residual: (writers, b, s, d)
        # Each slice along the first dimension represents one “writer” node’s hidden states.
        # inputs: x shape(b, s, d)
        # This is the MLP’s output for all tokens in the batch
        ...
        # get_mask(...) produces a binary (0/1) mask
        # z has shape (1, 1, 1)
        x = x * z
        # inputs: x shape(b, s, d)
        # If z == 0, the entire MLP output is zeroed out (i.e., “MLP node off”).
        # outputs: x shape(b, s, d)
        ...
        residual = residual + x
        # x.unsqueeze(0) changes x from (b, s, d) to (1, b, s, d), so there’s exactly one “slot” for this MLP node
        # self.mlp_write_common_mask is a buffer that indicates which writer index corresponds to the MLP.
        # For example, it is shaped (w, 1) (where w is the total number of writers), with a 1 in the row corresponding to the
        # MLP and 0s elsewhere.

        # when we call x.unsqueeze(0),
        # we are adding a new dimension of size 1 in front of the original tensor’s shape. Concretely:
        # inputs: x the MLP’s output
        # shape(b, s, d)
        # The newly added dimension (the 1 at the front) can be thought of as a "slot" or index to
        # indicate which node the (b, s, d) data belongs to. Here, we only have one MLP node for this
        # layer, so that extra dimension has size 1.

        # mlp_write_common_mask is z zero matrix of shape (w, 1) where w = num_writers
        # Exactly one row the row at self.mlp_writer_idx is set to 1;  all others remain 0

        # It acts as a fixed "placement" mask, telling the model which row (writer index) in the
        # (writers, batch, seq, hidden) tensor should store the MLP's output.

        # The letter i stands for the “node index” (here, i=1 for the MLP node).
        # Whichever row of self.mlp_write_common_mask is set to 1 determines where the MLP output lands in the final shape (w, b, s, d).
        # For example, if only row self.mlp_writer_idx is 1, then the MLP output goes to row self.mlp_writer_idx, and all other rows remain zero.

        # only one row in the (w,b,s,d) "multi-writer" tensor gets the MLP's output. The rest of the rows remain zero for that operation
        # one (b, s, d) slice placed into one row in (w, b, s, d)

        # residual
        # shape (writers, batch_size, sequence_length, hidden_size)
        # writers indexes all the “writer” nodes in the model (e.g., attention heads, the MLP, etc.).

        # residual is the big container of shape (writers, b, s, d) where each writer row can hold a node’s output.
        # x is the current node’s output reshaped to (writers, b, s, d)—usually with only one row nonzero.
        # residual = residual + x merges the node’s output into the shared tensor, so other parts of the model (or subsequent layers) can access it.

        return residual, torch.sum(z)


    # the MLP (like the attention block) has two different kinds of masking steps
    # mlp_read: Uses edge-level masks (via self.mlp_read_log_alphas) to decide which inputs (writer nodes) feed into the MLP.
    # mlp_write: Uses a node-level mask (via self.mlp_write_log_alphas) to decide whether the MLP as a whole is turned on or off.

    # They both call get_mask(...), but they’re masking different things:
    # Edge-level (which inputs go into the MLP),
    # Node-level (whether the MLP’s output is entirely off or on).

    #  they are complementary masks that happen at two stages:
    # First “read” from selected writers
    # Then optionally “write” the MLP result (or not) as a node.

    # mlp_read_log_alphas is a 1D vector of length num_writers, where each entry corresponds to exactly one possible writer→MLP edge.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # (w, h, b, s, h) # the collection of all writers
        #print(f"[DEBUG] Step 1: residual shape: {residual.shape}, device: {residual.device}, dtype: {residual.dtype}")
        ...
        # q_hidden_states is the post-masking and summation result.
        # It has already combined the contributions from whatever writer nodes were enabled for each attention head’s query
        # (num_heads, batch_size, sequence_length, hidden_size)
        # print(f"[DEBUG] hidden_states shape before self_attn: "
        #       f"{q_hidden_states.shape, k_hidden_states.shape, v_hidden_states.shape}")

        # print(f"[DEBUG] Step 2 (attn_read):")
        # print(f"q_hidden_states: shape={q_hidden_states.shape}, device={q_hidden_states.device}, dtype={q_hidden_states.dtype}")
        # print(f"k_hidden_states: shape={k_hidden_states.shape}, device={k_hidden_states.device}, dtype={k_hidden_states.dtype}")
        # print(f"v_hidden_states: shape={v_hidden_states.shape}, device={v_hidden_states.device}, dtype={v_hidden_states.dtype}")
        # print(f"z_attn_edges_sum: shape={z_attn_edges_sum.shape if isinstance(z_attn_edges_sum, torch.Tensor) else z_attn_edges_sum}")

        q_hidden_states = self.input_layernorm(q_hidden_states)
        # (num_heads, batch_size, sequence_length, hidden_size)
        #print(f"[DEBUG] Step 3 (q after LN): shape={q_hidden_states.shape}")

        k_hidden_states = self.input_layernorm(k_hidden_states)
        #print(f"[DEBUG] Step 4 (k after LN): shape={k_hidden_states.shape}")

        v_hidden_states = self.input_layernorm(v_hidden_states)
        #print(f"[DEBUG] Step 5 (v after LN): shape={v_hidden_states.shape}")

        # print(f"[DEBUG] hidden_states before self_attn: {q_hidden_states, k_hidden_states, v_hidden_states}")
        ...
        # hidden states shape output: (b, s, h)
        # print(f"[DEBUG] hidden_states after self_attn: {hidden_states}")
        #
        # print(f"[DEBUG] Step 6 (self_attn): hidden_states shape={hidden_states.shape}")

        # Step 7
        residual, z_attn_nodes_sum = self.attn_write(residual, hidden_states, corr_x=corr_x)
        # attn_write takes the new attention output (b, s, d) and accumulates it into the previous residual tensor (w, b, s, d)
        # The active heads’ outputs are then placed (via einsum) into the correct rows in the residual tensor, according to each head’s writer index.
        # residual = residual + x accumulates these new values on top of the previous residual, making them available as “writer nodes” for subsequent reads

        # Adding that to residual ensures row 𝑟
        # now contains the new data, whereas the rest of the rows remain unchanged from previous writes.


        #print(f"[DEBUG] Step 7 (attn_write): residual shape={residual.shape}, z_attn_nodes_sum={z_attn_nodes_sum}")


        # Step 8 # Fully Connected
        hidden_states, z_mlp_edges_sum = self.mlp_read(residual, corr_x=corr_x, embeds=embeds)
        # residual (w, b, s, d)
        # a final mask z (shape (w,)) that decides which writers feed into the MLP.
        # x_z = torch.einsum("wbsd,w->bsd", residual, z)
        # This operation sums over the writers dimension, weighting each writer’s hidden states by z[w]
        # If z[w] == 1, that writer’s outputs contribute; if 0, it does not.
        # The result x_Z is (b, s, d), a single hidden_state tensor that merges whichever writers are active
        # hidden_states (b, s, d) the combined input to the MLP
        # z_mlp_edges_sum: the total number of active edges from the writer dimension

        #print(f"[DEBUG] Step 8 (mlp_read): hidden_states shape={hidden_states.shape}, z_mlp_edges_sum={z_mlp_edges_sum}")

        # Step 9
        hidden_states = self.post_attention_layernorm(hidden_states)
        # (b, s, d)
        #print(f"[DEBUG] Step 9 (post_attn_LN): hidden_states shape={hidden_states.shape}")

        # print(f"[DEBUG] hidden_states shape before mlp={hidden_states.shape}")
        # print(f"[DEBUG] hidden_states before mlp: {hidden_states}")
        # Step 10
        hidden_states = self.mlp(hidden_states)
        # input: hidden_states (b, s, d)
        # output: (b, s, d)

        # print(f"[DEBUG] hidden_states shape after mlp={hidden_states.shape}")
        # print(f"[DEBUG] hidden_states after mlp: {hidden_states}")
        #print(f"[DEBUG] Step 10 (mlp): hidden_states shape={hidden_states.shape}")

        # Step 11
        hidden_states, z_mlp_nodes_sum = self.mlp_write(residual, hidden_states, corr_x=corr_x)

        # input: residual (w, b, s, d)
        # hidden_states The MLP’s new output of shape (b, s, d)
        # self.mlp_write_log_alphas is a parameter (or small vector) that determines if the entire MLP node is active (1) or off (0).
        # x = torch.einsum("ibsd,wi->wbsd", x.unsqueeze(0), self.mlp_write_common_mask) (w, b, s, d)
        # residual = residual + x   Collects the new MLP output into the multi-writer tensor.
        # output: The updated multi-writer residual (w, b, s, d)

        #print(f"[DEBUG] Step 11 (mlp_write): hidden_states shape={hidden_states.shape}, z_mlp_nodes_sum={z_mlp_nodes_sum}")

        # Step 12
        z_edges_sum = z_attn_edges_sum + z_mlp_edges_sum
        z_nodes_sum = z_attn_nodes_sum + z_mlp_nodes_sum
        #print(f"[DEBUG] Step 12: z_edges_sum={z_edges_sum}, z_nodes_sum={z_nodes_sum}")
        ...
        #print(f"[DEBUG] Step 13 (output): Returning hidden_states shape={hidden_states.shape}")
        return outputs
...
class FLlamaModel(FLlamaPreTrainedModel):
    ...
    def set_edge_threshold_for_deterministic(self, edge_threshold_for_deterministic):
        self.edge_threshold_for_deterministic = edge_threshold_for_deterministic
        # that determines the threshold used for deterministic edge gating (pruning).
        for layer in self.layers:
            layer.set_edge_threshold_for_deterministic(edge_threshold_for_deterministic)
            # ensures all layers use the same threshold logic when creating their binary masks for edges.

    @torch.no_grad()
    def set_node_threshold_for_deterministic(self, node_threshold_for_deterministic):
        self.node_threshold_for_deterministic = node_threshold_for_deterministic
        # that determines the threshold used for deterministic node gating (pruning).
        for layer in self.layers:
            layer.set_node_threshold_for_deterministic(node_threshold_for_deterministic)
            # ensures all layers use the same threshold logic when creating their binary masks for nodes.
    ...
    def get_edge_masks(self):
        ...
        return masks
    # Each layer’s edge masks returned by layer.get_edge_masks()
    # Each layer’s edge masks include:
    # The Q/K/V read masks (e.g., q_read_log_alphas, k_read_log_alphas, v_read_log_alphas)
    # The MLP read masks (e.g., mlp_read_log_alphas)
    # The “final read” mask from self.final_read_log_alphas, which determines how the final hidden states are read.

    # get_edge_masks returns a list of edge masks for all layers plus the final read edge, letting you inspect or manipulate the binary gating at the edge level throughout the whole model.
    ...
    # In each FLlamaDecoderLayer,
    # node masks typically correspond to the entire attention heads (taken together) and the MLP as “nodes.”
    # Attention node mask:
    # Controlled by self_attn_write_log_alphas
    # It’s a vector of length num_heads, where each entry decides if a particular head is “on” (1) or “off” (0).

    # MLP node mask:
    # controlled by self_mlp_write_log_alphas
    # Usually just one entry (or a small vector) indicating whether the entire MLP block is active.

    # Each layer’s masks are appended to the masks list
    # returns the collected list of all node masks which includes the
    # optional embedding node mask (if present) plus each layer’s node masks.
    ...
    def forward(
        ...
        #("FLlama position_ids:", position_ids[0, :10])
        ...
        #print("AFTER READ: hidden_states.shape =", hidden_states.shape)
...
class FLlamaForCausalLM(FLlamaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = outputs[0] # THE last_hidden_state from FLlamaModel
        #print("[DEBUG] final hidden_states", hidden_states.shape)
    ...
    def prepare_inputs_for_generation(
        ...
        # print("[DEBUG] Called prepare_inputs_for_generation()")
        ...
        # print(f"[DEBUG] cache_position: {cache_position}")
        ...
        # print("[DEBUG] final model_inputs keys:", model_inputs.keys())
